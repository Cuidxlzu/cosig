...
\begin{itemize}
    ...
    \item \textbf{There can be false negatives.} A tool may not flag image features that are indicative of image integrity issues. For instance, software for detecting image duplications within a figure may miss some duplications that are visible by eye. Several tools are prone to missing image duplications in low-contrast images. It is also possible that a tool will flag some duplications while missing others in the same image.
