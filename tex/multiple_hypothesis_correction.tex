...
\cosigsection{Multiple hypothesis correction}{9 March 2026}
...
$p$ values from Bonferroni correction will accurately correct for FWER if the variables under consideration are independent. However, if the variables are correlated, Bonferroni correction will be overconservative. In this case, other methods for correction that take into account the correlation structure are likely preferable.
...
Note that $q$ values derived from the Benjamini-Hochberg procedure should not be confused with \href{https://doi.org/10.1111/1467-9868.00346}{Storey's $q$ values}, which are derived from a similar but mathematically distinct FDR-controlling procedure. See \href{https://doi.org/10.1038/nmeth.2900}{Krzywinski and Altman (2014)} for a comparison of these methods.
...
\subsection*{Example 2: Inconsistency of $p$ values and $q$ values}
...
\subsection*{Example 3: Lack of multiple hypothesis correction}
\href{https://doi.org/10.3390/nu17091602}{Zhang et al (2025)} study the effect of folinic acid on autistic children but note that ``this study did not perform statistical corrections for multiple testing, which may result in false-positive results''. The abstract of the paper does not mention this limitation and treats the results as ``promising''. However, no results presented in Table 2 or Table 3 remain statistically significant after any common multiple hypothesis correction procedure is performed. As noted \href{https://pubpeer.com/publications/70F34940FF5D7EFD14E1B1B186431B}{on PubPeer}, it is not uncommon to treat multiple hypothesis correction as optional even when analytical results depend heavily on its implementation.

\subsection*{Example 4: Lack of multiple hypothesis correction}

\href{https://doi.org/10.4137/iii.s25105}{Marshall-Gradisnik et al. (2015)} analyze 393 single nucleotide polymorphisms and find 19 statistically significant correlations.
However, as pointed out \href{https://www.s4me.info/threads/examination-of-single-nucleotide-polymorphisms-in-acetylcholine-receptors-in-chronic-fatigue-syndrome-patients-2015-marshall-gradisnik-et-al.36899/}{on the ``Science for ME'' forum} and \href{https://pubpeer.com/publications/49C2FDD37CE6BCEB7B47D8062F4BD4}{on PubPeer}, one would expect an average of 19.65 false positives if the null hypothesis were true.
After correcting for multiple hypotheses, none of the correlations are statistically significant.
...
\begin{itemize}
    ...
    \item \href{https://doi.org/10.1038/nmeth.2900}{``Comparing samples--Part II'' (2014)}
\end{itemize}
