...
\cosigpart{Basics}
\cosigsection{Getting started with post-publication peer review}{10 March 2026}
...
\begin{enumerate}
    ...
    \item Create a \href{https://pubpeer.com}{PubPeer} account.
    \item Install \href{https://pubpeer.com/static/extensions}{PubPeer's browser extension}.
    \item Start by verifying fingerprints the \href{https://www.irit.fr/~Guillaume.Cabanac/problematic-paper-screener/}{Problematic Paper Screener} has detected.
...
\href{https://en.wikipedia.org/wiki/Open_access}{``Open access''} articles can be read in full by anyone with an internet connection at no cost. Not all articles are open access; many require a subscription to the publisher to read in full, typically provided to academics by their institution. 
If you do not have an institutional subscription, you can usually obtain a copy of the article by sending an email request to the corresponding author. Where this is not possible (because you get no response, or because you wish to remain anonymous) you may only be able to read title, abstract and author information without cost, often leaving the full text, author declarations and supplementary materials inaccessible. This depends on the article's publisher and when the article was published.
...
Consider installing \href{https://pubpeer.com/static/extensions}{PubPeer's browser extension} (available for Chrome, Firefox, Edge and Safari), which highlights when articles on a given webpage have existing PubPeer comments.
...
\begin{enumerate}
    ...
    \item Preview to ensure that the comment is formatted correctly, then post!
    \item PubPeer will open a window where you can emails for authors of the commented article. This may be populated automatically, but it is always worth adding emails for at least one author, and preferably more. If the author email is left blank, authors may be unaware of the PubPeer comment, whereas any authors added here will receive an automatic notification. This gives them the opportunity to reply. Note that under PubPeer's current policy, non-institutional email addresses entered by users are rarely accepted.
\end{enumerate}
