...
Authors are the best experts on their own publications and PubPeer has the option to provide author emails to loop them into a discussion (note that under PubPeer's current policy, non-institutional email addresses entered by users are rarely accepted). Questions about a publication can often be addressed directly by authors, so it is helpful to make clarifying requests where appropriate.
